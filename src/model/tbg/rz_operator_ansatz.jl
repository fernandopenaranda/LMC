...
function non_local(p, k)
    ...
    for n in [0,1,2] # first neighbours
        a_n = mod_a_n .* [real(exp(1im*(n*2π/3+π/2))), imag(exp(1im*(n*2π/3+π/2)))] # three real space vectors perpendicular to the moir'e momentum transfer
        val += sin(k' * a_n)
    end
...
#sztz 3.054/2.4083377049259522=1.2680945839752567 with sz=1 closing at K
