...
""" 
    `Emin_μαs(p::Planar_σijk_presets, μ0s::Vector{Int64}, μs::Union{Array,AbstractRange};
        μ = 0, U = 0, λ = 0, evals = 1e2, η = 0.05, estimated_bound_width = 10, iterations = 100) `
...
function Emin_μαs(p::Planar_σijk_presets, μ0s::Array, μ::Number;
    U = 0, J = 0, λ = 0, evals = 1e2, η = 0.05, estimated_bound_width = 10, iterations = 100, int_model = :SU4) 
...
function Emin_μαs(p::Planar_σijk_presets, μ::Number; random_guesses = 2,
    U = 0, J = 0, λ = 0, evals = 1e2, η = 0.05, estimated_bound_width = 10, iterations = 100, int_model = :SU4) 
    ...
    ϵ_range, int_dos = interpolated_dos(p; evals = evals, η = η)
    n = interpolated_n(int_dos, ϵ_range)
    E0 = total_energy(ones(4) .* μ, int_dos, n, 0, 0, :SU4) #non_interacting energy
    ...
    while count < random_guesses
        μ0s = rand(4) * 2μ
        μαs = opt_μs(μ0s, n; μ = μ, U = U, J = J, λ = λ, 
        evals = eval, η = η, estimated_bound_width = estimated_bound_width, iterations = iterations, int_model = int_model)
        # println(μαs)
        E = total_energy(μαs, int_dos, n, U, J, int_model) # compute the total energy
        println("guess number: ", count, " E: ", E)
        push!(μα_mat, μαs)
        append!(Es, E)
        count +=1 
    end
...
or 
    `opt_μs(μ0s::Vector{Float64}, n::ScaledInterpolation;
        μ = 0, U = 0, λ = 0, evals = 1e2, η = 0.05, estimated_bound_width = 10, iterations = 100)`

method of bandstructures with a Hartree local potential
here we find the extrema of the grand functional: Φ/A = ∑_iE_α(μ_α) + V_Hartree - TS - μ ∑_α n_α.
Its minima in terms of μ_α a 4-dimensional array corresponding to the chemical potentials of each of the 4 flavors

So we take the derivatives of the grand functional which lead (even at finite T) to the following extrema equations
F(i) = μ_i - μ + U A * ∑_β≠α n_β = 0 with i and j in 1:4. And reformulate them as an optimization problem so 
∑_i(F(i)^2) = 0. This is the quantity to optimize in terms of μ_i, not that n_β = ∫₀^β ρ(ϵ) dϵ. Since ρ(ϵ) the dos is
the same for the four flavours in this rigid band (local (q independent) Hartree perturbation) we first create this function
and interpolate it. We also interpolate the density of states for conveniency
μ0s are the seeds

Attention there is a field inside the Planar_σijk_presets.computation that is evals, we do not use it here but you should
change the kws to be consistent in future calculations. Also allow for the Dos struct.
"""
opt_μs(p::Planar_σijk_presets, μ0s::Vector{Int64}; evals = 10, η = 0.05, kws...) = 
    opt_μs(μ0s, interpolated_dos(p; evals = evals, η = η); kws...)
...
function opt_μs(μ0s::Vector{Float64}, n::ScaledInterpolation;
    μ = 0, U = 0, J = 0, λ = 0, evals = 1e2, η = 0.05, estimated_bound_width = 10, iterations = 100, int_model = :SU4)
...
function opt_μs(objective, n0, n, μ0s, μ, U, J, λ, lower, upper, iterations)
    result = optimize(μs -> objective(n, μs, μ, U, J, λ),
            ...
            μ0s,   # seed
            Fminbox(BFGS()), # boxed interaction limited to small μvalues determined by lower and upper and within the interpolation 'mulist range
...
""" reformulation of the root-finder problem intro an optimization least squares one introducing
a su4 preserving MF decoupling corresponding to Vint = UA/2 ∑_α≠β n_α n_β"""
...
""" reformulation of the root-finder problem intro an optimization least squares one introducing
a su2 preserving MF decoupling corresponding to Vint = UA/2 ∑_α≠β n_α n_β +  JAu.c.(n1 − n3)(n2 − n4)
...
""" the chemical potentials resulting from the optimization should obey that ∑n_α(U, J) = 4n0 with n0 the non-interacting density (equal to the four flavours). we do this by a lagrange multiplier"""
...
"""
function interpolated_dos(p::Planar_σijk_presets; μlist = -10.0:0.1:10.0, η = 0.05, evals = 100) 
...
interpolated_n(int_dos, x::Array) = interpolated_n(int_dos, range(first(x), last(x); length=length(x)))
