...
# function P_proj(p::ParamsBM, k = [0.,0]) # possibly take k = 0 (c-electrons doesn't matter away from Gamma)
#     _, evecs = eigen(Matrix(bistritzer_hamiltonian(p,k)))
#     P = zeros(ComplexF64, size(evecs,1), size(evecs,1))
#     half_dim = size(evecs,1)÷2
#     lowest_evecs = [evecs[:,i] for i in half_dim-2:half_dim+3] # only six bands are into the Wannierization
#     for evec in lowest_evecs
#         P .+= conj.(evec) * transpose(evec) 
#     end
#     P
# end
...
function Oc(p, obs, k)
    ...
    # if minimum(abs.(F.values[sorted_indices])) < 0 ||  minimum(abs.(F.values[sorted_indices])) > 1-1e-5
    #     @warn "Error in the estimation of u_Qβ"
    println(minimum(abs.(F.values[sorted_indices])))
    # end   
...
# Of(pbm, diagm(u), rcp.κ2)

# EXTRA NUMERICAL INTEGRATION DEPRECATED CAUSE I FOUND AN ANALYTICAL WAY

# ω11(l, r, p) = α1/√2 * 1/√(π*λ1(p)^2) * exp(1im*π/4*l*p.ν -norm(r)^2/(2λ1(p)^2))
# ω21(l, r, p) = -l * α2/√2 * (r[1]+1im*p.ν*r[2])/(√π*λ2(p)^2) *
#      exp(1im*π/4*l*p.ν -norm(r)^2/(2λ2(p)^2))
# ω12(l, r, p) = l * α2/√2 * (r[1]-1im*p.ν*r[2])/(√π*λ2(p)^2) * 
#     exp(-1im*π/4*l*p.ν -norm(r)^2/(2λ2(p)^2))
# ω22(l, r, p) = α1/√2 * 1/√(π*λ1(p)^2) * exp(-1im*π/4*l*p.ν -norm(r)^2/(2λ1(p)^2))

# function is_inside_hexagon(x)
#     λ = a0/(2pi*p.θ/(2*360))
#     θs = [π/3 * k for k in 0:5]
#     for θ in θs
#         if dot([x[1], x[2]], [cos(θ), sin(θ)]) > λ * cos(π/6)
#             return false
#         end
#     end
#     return true
...
# "numerical integration in r. Deprecated because I have found the analytical integrals see below."
# function v(p, k, η, l, ωij::Function)
#     λ = a0/(2pi*p.θ/(2*360)) # Moiré length scale (lattice constant of the localized Wannier orbitals in the triangular lattice)
#     unitcell_area = 3√3/2 * λ^2 # hexagon area
#     integrand(x,y) = ωij(l, [x,y], p) * exp(-1im *(k' * [x,y])) 
#     reltol = 1e-3 # generalize to include G
#     integral_y, error_y = quadgk(y -> quadgk(x -> integrand(x, y), -Inf, Inf, maxevals = 1000)[1], -Inf, Inf, maxevals = 1000)
#     return integral_y/unitcell_area, error_y
# end
