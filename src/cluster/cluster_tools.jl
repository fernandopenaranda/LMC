...
function slurm_submit_phasediagrams(interpolationsPID::Union{String,Number};
    ...
    println("bash_file_name: ", script)
    cmd = `sbatch $script $lmcfolder $interpolationsPID $Ezsteps $nu_min $nu_max $nu_points $U $J $lambda $(0.05) $estimated_bound_width $iterations $int_mode $random_guesses`
    dryrun && return cmd
    run(cmd)
end